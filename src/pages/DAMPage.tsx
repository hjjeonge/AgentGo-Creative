...
const toStringRecord = (metadata: Record<string, unknown> | null | undefined): Record<string, string> => {
  if (!metadata) return {};
...
const getMetadataValue = (metadata: Record<string, unknown> | null | undefined, keys: string[]): string | undefined => {
...
const parseAiTags = (metadata: Record<string, unknown> | null | undefined): DAMFile["aiTags"] => {
...
const parseWorkfront = (metadata: Record<string, unknown> | null | undefined): DAMFile["workfront"] => {
  if (!metadata) return undefined;
  const projectName = getMetadataValue(metadata, ["workfront_project_name", "wf_project_name"]);
  const projectId = getMetadataValue(metadata, ["workfront_project_id", "wf_project_id"]);
  const documentId = getMetadataValue(metadata, ["workfront_document_id", "wf_document_id"]);
  const taskName = getMetadataValue(metadata, ["workfront_task_name", "wf_task_name"]);
  const taskDescription = getMetadataValue(metadata, ["workfront_task_description", "wf_task_description"]);
  if (!projectName && !projectId && !documentId && !taskName && !taskDescription) return undefined;
  return {
...
const toFileFromDetail = (detail: Awaited<ReturnType<typeof getAssetDetail>>): DAMFile => {
  ...
  return {
    ...
    expirationDate: getMetadataValue(metadata, ["expiration_date", "expirationDate"]),
    gps: {
    ...
    referenceImages: detail.reference_images?.filter((image): image is string => typeof image === "string"),
  };
};
...
const MOCK_COLLECTION_NAMES = ["Summer Campaign", "Ski Season 2026", "Modern Interior"];

export const DAMPage: React.FC = () => {
  const [files, setFiles] = useState<DAMFile[]>([]);
  const [tasks, setTasks] = useState<DAMTask[]>([]);
  const [collections, setCollections] = useState<CollectionItem[]>([]);
  const [collectionShareUrl, setCollectionShareUrl] = useState<string | null>(null);
  ...
  const [filters, setFilters] = useState<FilterState>({
    fileType: null, person: null, dateFrom: null, dateTo: null, status: null, mimeTypes: [],
  });

  const isWorkspaceView = activePath === "My Workspace";
  const isCollectionView = activePath.startsWith("coll:");
  const activeCollectionId = isCollectionView ? activePath.replace("coll:", "") : "";
  ...
  useEffect(() => {
    getFolderTree().then(setFolders).catch(() => setFolders([]));
    listWorkspaceTasks()
      .then((rows) => {
    ...
    listCollections()
      .then(setCollections)
      .catch(() => {
        setCollections(MOCK_COLLECTION_NAMES.map((name, index) => ({ id: `fallback-${index}`, name, asset_ids: [] })));
      });
  }, []);
  ...
  useEffect(() => {
    ...
    const isSpecial = specialPaths.includes(activePath) || projectCategories.includes(activePath) || activePath.startsWith("coll:");
    ...
    listAssets({
      ...
      .then((response) => {
        setFiles(response.items.map((asset) => ({
          id: asset.id,
  ...
  const filteredFiles = files.filter((file) => {
    if (isCollectionView && activeCollection && activeCollection.asset_ids.length > 0 && !activeCollection.asset_ids.includes(file.id)) {
      return false;
    }
    if (filters.status && file.status !== filters.status) return false;
    if (filters.mimeTypes.length > 0 && !filters.mimeTypes.includes(getFileExtension(file.name))) return false;
    return true;
  });
  ...
  else if (isCollectionView) breadcrumbs = ["DAM", "Collections", activeCollection?.name || activeCollectionId];
  else if (activePath !== "DAM") breadcrumbs = ["DAM", folderMap.get(activePath) || activePath];
  ...
  const handleRenameConfirm = async (newName: string) => {
    ...
    try {
      ...
      setFiles((prev) => prev.map((file) => (file.id === renamingFile.id ? { ...file, name: newName } : file)));
  ...
  const handleContextAction = async (action: string, fileId: string) => {
    ...
    if (action === "delete" && window.confirm(`"${file.name}"을 삭제하시겠습니까?`)) {
      try {
  ...
  const handleSaveNewFile = async (metadata: Record<string, string>, uploadedFile: File | null) => {
    ...
    const asset = await uploadAsset(uploadedFile, uploadedFile.name, metadata, folderId);
    setFiles((prev) => [
      {
        id: asset.id,
        type: toFileType(asset.file_type),
  ...
  const handleWorkspaceTaskStatus = async (taskId: string, status: DAMTask["status"]) => {
  ...
  return (
    <div className="h-full flex overflow-hidden bg-white">
      <DAMSidebar activePath={activePath} onNavigate={setActivePath} folders={folders} collections={collections} />
      ...
      <div className="flex-1 flex flex-col overflow-hidden border-l border-[#E2E8F0]">
        <div className="flex items-center gap-[16px] px-[24px] py-[16px] border-b border-[#E2E8F0] bg-white z-10">
          {!isAdvancedFilterOpen && !isWorkspaceView && (
            <button onClick={() => setIsAdvancedFilterOpen(true)} className="p-[8px] rounded-[6px] hover:bg-[#F1F5F9] transition-colors" title="필터 열기">
          ...
          <div className="flex-1 flex items-center gap-[8px] text-[16px] font-semibold text-[#0F172B] truncate">
            {breadcrumbs.map((crumb, index) => (
              <span key={crumb} className="flex items-center gap-[8px]">
                {index > 0 && <span className="text-[#94A3B8] font-normal">&gt;</span>}
                <span className={index < breadcrumbs.length - 1 ? "text-[#475569] font-normal text-[14px]" : ""}>{crumb}</span>
              </span>
            ))}
          </div>
          ...
          {!isWorkspaceView && (
            <div className="flex items-center gap-[4px]">
              <button onClick={() => setViewMode("grid")} className={`w-[32px] h-[32px] flex items-center justify-center rounded-[6px] ${viewMode === "grid" ? "bg-[#EFF6FF]" : "hover:bg-[#F1F5F9]"}`}>
                <GridViewIcon active={viewMode === "grid"} />
              </button>
              <button onClick={() => setViewMode("list")} className={`w-[32px] h-[32px] flex items-center justify-center rounded-[6px] ${viewMode === "list" ? "bg-[#EFF6FF]" : "hover:bg-[#F1F5F9]"}`}>
                <ListViewIcon active={viewMode === "list"} />
              </button>
            </div>
          )}

          <button onClick={() => setShowNewModal(true)} className="bg-[#155DFC] text-white px-[16px] py-[8px] rounded-[8px] text-[14px] font-medium whitespace-nowrap active:scale-[0.98] transition-transform shadow-sm">
        ...
        {isWorkspaceView ? (
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto bg-[#F8FAFC]/30">
              {loading ? (
                <div className="flex items-center justify-center h-full text-[14px] text-[#94A3B8]">에셋 불러오는 중...</div>
              ) : filteredFiles.length === 0 ? (
                <div className="flex items-center justify-center h-full text-[14px] text-[#94A3B8]">표시할 파일이 없습니다.</div>
              ) : viewMode === "grid" ? (
                <DAMGridView files={filteredFiles} onContextMenu={handleContextMenu} onDotsClick={handleDotsClick} onDownload={handleDownload} onDelete={(id) => handleContextAction("delete", id)} />
              ) : (
                <DAMListView files={filteredFiles} onContextMenu={handleContextMenu} onDotsClick={handleDotsClick} onDownload={handleDownload} onRename={(id) => setRenamingFile(files.find((file) => file.id === id) || null)} />
              )}
            </div>
          </>
        )}
      ...
      {!isWorkspaceView && (
        <DAMCreativePanel files={filteredFiles} isOpen={isCreativePanelOpen} onToggle={() => setIsCreativePanelOpen((prev) => !prev)} />
      )}
      ...
      {renamingFile && <RenameModal defaultValue={renamingFile.name} onConfirm={handleRenameConfirm} onCancel={() => setRenamingFile(null)} />}
      {showNewModal && <NewFileModal onClose={() => setShowNewModal(false)} onSave={handleSaveNewFile} />}
      {detailFile && <FileDetailModal file={detailFile} onClose={() => setDetailFile(null)} onOpenPermissions={(file) => setPermissionFile(file)} />}
      {permissionFile && <PermissionModal file={permissionFile} onClose={() => setPermissionFile(null)} />}
    </div>
  );
};
