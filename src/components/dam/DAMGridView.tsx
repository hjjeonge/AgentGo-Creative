...
export const DAMGridView: React.FC<Props> = ({ files, onContextMenu, onDotsClick, onDownload, onDelete }) => {
      ...
      {files.map((file) => (
        ...
        >
          ...
          <div className="relative aspect-square bg-[#F8FAFC] rounded-[8px] flex items-center justify-center overflow-hidden">
            ...
            {/* 호버 시 퀵액션 오버레이 */}
            <div className={`absolute inset-0 bg-black/20 flex items-center justify-center gap-[10px] transition-opacity duration-200 ${hoveredId === file.id ? "opacity-100" : "opacity-0"}`}>
              <button 
                ...
                title="다운로드"
              >
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-[#475569] group-hover/btn:text-white">
            ...
            {file.status && file.status !== "none" && (
              <div className={`absolute bottom-[8px] right-[8px] inline-flex items-center gap-[4px] px-[8px] py-[3px] rounded-[4px] text-[10px] font-bold uppercase text-white shadow-sm ${getStatusColor(file.status)}`}>
