...
const FILE_TYPE_ICONS: Record<string, React.ReactNode> = {
  folder: (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
      <path d="M1 3.5C1 3.22 1.22 3 1.5 3H4.8L5.8 4H12.5C12.78 4 13 4.22 13 4.5V11C13 11.28 12.78 11.5 12.5 11.5H1.5C1.22 11.5 1 11.28 1 11V3.5Z" fill="#3B82F6" />
    </svg>
  ),
  video: (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
  ...
  pdf: (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
      ...
      <text x="3.5" y="10" fontSize="4" fill="white" fontWeight="bold">PDF</text>
    </svg>
  ),
  zip: (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
  ...
  image: (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
      ...
      <path d="M1 10L4 7L6.5 9.5L9 7L13 10" stroke="#CBD5E1" strokeWidth="1" fill="none" />
    </svg>
  ),
  other: (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
...
const MiniCalendar: React.FC<CalendarProps> = ({ value, onChange }: CalendarProps) => {
  ...
  return (
    <div className="bg-white border border-[#E2E8F0] rounded-[8px] shadow-md p-[12px] w-[200px]">
      ...
      <div className="grid grid-cols-7 gap-[2px] mb-[4px]">
        {DAYS.map((d) => (
          <div key={d} className="text-center text-[10px] text-[#94A3B8] py-[2px]">{d}</div>
        ))}
      </div>
  ...
  return (
    <div ref={wrapRef} className="flex items-center gap-[8px] relative">
      ...
      <div className="relative">
        ...
        >
          파일 유형
          <img src={ArrowDown} className={`w-[10px] h-[10px] transition-transform ${open === "fileType" ? "rotate-180" : ""}`} />
        </button>
        {open === "fileType" && (
          <div className="absolute top-[calc(100%+4px)] left-0 z-[50] bg-white border border-[#E2E8F0] rounded-[8px] shadow-lg py-[4px] min-w-[120px]">
            {FILE_TYPES.map((ft) => (
              <button
                key={ft.value}
                onClick={() => {
                  onFiltersChange({ ...filters, fileType: filters.fileType === ft.value ? null : ft.value });
                  setOpen(null);
                }}
                className={`w-full flex items-center gap-[8px] px-[14px] py-[8px] text-[13px] hover:bg-[#F1F5F9] ${
                  filters.fileType === ft.value ? "text-[#155DFC] bg-[#EFF6FF]" : "text-[#0F172B]"
                }`}
              >
      ...
      <div className="relative">
        ...
        >
          사람
          <img src={ArrowDown} className={`w-[10px] h-[10px] transition-transform ${open === "person" ? "rotate-180" : ""}`} />
        </button>
        {open === "person" && (
      ...
      <div className="relative">
        ...
        >
          수정 날짜
          <img src={ArrowDown} className={`w-[10px] h-[10px] transition-transform ${open === "date" ? "rotate-180" : ""}`} />
        </button>
        {open === "date" && (
              ...
              {DATE_PRESETS.map((preset) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (preset === "오늘") from = now.toISOString().slice(0, 10);
                    ...
                    onFiltersChange({ ...filters, dateFrom: from, dateTo: now.toISOString().slice(0, 10) });
                    setOpen(null);
                  }}
              ...
              <div className="ml-[4px] bg-white border border-[#E2E8F0] rounded-[8px] shadow-lg p-[12px]">
                <div className="flex flex-col gap-[8px] mb-[10px]">
                  {(["from", "to"] as const).map((mode) => (
                    ...
                    >
                      <span className="text-[#94A3B8]">{mode === "from" ? "이후" : "이전"}</span>
                      <span className="flex-1 text-left text-[#0F172B]">
                        {mode === "from" ? (filters.dateFrom ?? "") : (filters.dateTo ?? "")}
                      </span>
                      <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                        <rect x="1" y="2" width="12" height="11" rx="1.5" stroke="#94A3B8" strokeWidth="1.2" />
                ...
                <MiniCalendar
                  value={dateCalendarMode === "from" ? (filters.dateFrom ?? "") : (filters.dateTo ?? "")}
                  onChange={(date) => {
                    if (dateCalendarMode === "from") onFiltersChange({ ...filters, dateFrom: date });
