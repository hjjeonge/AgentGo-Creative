...
import { listAssetPermissions, saveAssetPermissions, type AssetPermission } from "../../services/dam";
...
export const PermissionModal: React.FC<Props> = ({ file, onClose }: Props) => {
  ...
  const [selectedLevel, setSelectedLevel] = useState<UserPermission["accessLevel"]>("can-view");
  ...
  useEffect(() => {
    listAssetPermissions(file.id)
      ...
      .catch(() => {
        setPermissions([{ id: "owner", name: "Admin", email: "admin@itcen.com", accessLevel: "owner" }]);
      });
  }, [file.id]);
  ...
  const handleLevelChange = (id: string, level: string) => {
    setPermissions((prev) => prev.map((p) => (
      p.id === id ? { ...p, accessLevel: level as UserPermission["accessLevel"] } : p
    )));
  };
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40 backdrop-blur-[2px]">
      <div className="bg-white rounded-[16px] shadow-2xl w-[600px] max-h-[85vh] flex flex-col overflow-hidden animate-in zoom-in duration-150">
        <div className="flex items-center justify-between p-[20px_24px] border-b border-[#E2E8F0] bg-[#F8FAFC]">
          <div>
            <h2 className="text-[16px] font-bold text-[#0F172B]">Manage Permissions</h2>
            <p className="text-[12px] text-[#94A3B8] truncate max-w-[400px]">/DAM/{file.name}</p>
          </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="p-[24px] border-b border-[#F1F5F9] bg-white flex items-center gap-[12px]">
            ...
            <select
              value={selectedLevel}
              onChange={(e) => setSelectedLevel(e.target.value as UserPermission["accessLevel"])}
                ...
                <div key={permission.id} className="flex items-center justify-between group">
                  <div className="flex items-center gap-[12px]">
                    ...
                    <div className="flex flex-col">
                      <span className="text-[13px] font-bold text-[#0F172B]">{permission.name}</span>
                      <span className="text-[11px] text-[#94A3B8]">{permission.email}</span>
                    </div>
                  </div>

                  <div className="flex items-center gap-[12px]">
                    {permission.accessLevel === "owner" ? (
                      <span className="text-[12px] text-[#94A3B8] font-medium px-[8px]">Owner</span>
                    ) : (
                      <div className="flex items-center gap-[8px]">
                        <select
                          value={permission.accessLevel}
                          onChange={(e) => handleLevelChange(permission.id, e.target.value)}
                          className="bg-transparent text-[12px] text-[#475569] outline-none border-none hover:text-[#155DFC] transition-all text-right"
                        >
                          {ACCESS_LEVELS.filter((level) => level.value !== "owner").map((level) => (
                            <option key={level.label} value={level.value}>{level.label}</option>
                          ))}
                        </select>
                        <button
        ...
        <div className="p-[20px_24px] border-t border-[#E2E8F0] flex justify-end bg-[#F8FAFC]">
          <button
            ...
            className={`px-[32px] py-[10px] text-white text-[14px] font-bold rounded-[8px] shadow-md transition-all active:scale-[0.98] ${
              saving ? "bg-[#94A3B8] cursor-not-allowed" : "bg-[#155DFC] hover:bg-[#155DFC]/90"
            }`}
          >
            확인
          </button>
