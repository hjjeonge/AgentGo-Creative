...
export const DAMAdvancedFilters: React.FC<Props> = ({ filters, onFiltersChange, isOpen, onToggle }: Props) => {
  const [expanded, setExpanded] = useState<string[]>(["STATUS", "TYPE", "MIME"]);
  ...
  return (
    <aside className="w-[220px] shrink-0 bg-white border-r border-[#E2E8F0] overflow-y-auto flex flex-col">
      <div className="px-[16px] py-[14px] border-b border-[#E2E8F0] flex items-center justify-between">
        <span className="text-[14px] font-semibold text-[#0F172B]">FILTERS</span>
        <button onClick={onToggle} className="text-[#94A3B8] text-[12px] hover:text-[#0F172B]">
          Hide
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="border-b border-[#F1F5F9]">
          ...
          >
            <span className="text-[12px] font-semibold text-[#475569] tracking-tight">ASSET STATUS</span>
            <img
              src={ArrowDown}
          ...
          {expanded.includes("STATUS") && (
            <div className="px-[16px] pb-[16px] flex flex-col gap-[8px]">
              {ASSET_STATUS.map((s) => (
                <label key={s.value} className="flex items-center gap-[8px] cursor-pointer group">
                  <div className="relative flex items-center justify-center">
                    <input
                      type="radio"
                      name="status"
                      checked={filters.status === s.value || (!filters.status && s.value === "all")}
                      onChange={() => onFiltersChange({ ...filters, status: s.value === "all" ? null : s.value })}
                  ...
                  <span className="text-[13px] text-[#475569] group-hover:text-[#0F172B]">{s.label}</span>
                </label>
              ))}
            </div>
          )}
        ...
        <div className="border-b border-[#F1F5F9]">
          <button
            ...
            className="w-full px-[16px] py-[12px] flex items-center justify-between group"
          >
            <span className="text-[12px] font-semibold text-[#475569] tracking-tight">FILE TYPE</span>
            <img
              src={ArrowDown}
              className={`w-[10px] h-[10px] transition-transform ${
          ...
          {expanded.includes("TYPE") && (
            <div className="px-[16px] pb-[16px] flex flex-col gap-[8px]">
              {FILE_TYPES.map((t) => (
                <label key={t.value} className="flex items-center gap-[8px] cursor-pointer group">
                  <div className="relative flex items-center justify-center">
                    <input
                      type="checkbox"
                      checked={filters.fileType === t.value}
                      onChange={() => onFiltersChange({ ...filters, fileType: filters.fileType === t.value ? null : t.value })}
                  ...
                  <span className="text-[13px] text-[#475569] group-hover:text-[#0F172B]">{t.label}</span>
                </label>
              ))}
            </div>
          )}
        ...
        <div>
          <button
            ...
            className="w-full px-[16px] py-[12px] flex items-center justify-between group"
          >
            <span className="text-[12px] font-semibold text-[#475569] tracking-tight">MIME TYPE</span>
            <img
              src={ArrowDown}
              className={`w-[10px] h-[10px] transition-transform ${
          ...
          {expanded.includes("MIME") && (
            <div className="px-[16px] pb-[16px] grid grid-cols-2 gap-[8px]">
              {MIME_TYPES.map((m) => (
                <label key={m} className="flex items-center gap-[6px] cursor-pointer group">
                  ...
                  <span className="text-[12px] text-[#475569] group-hover:text-[#0F172B]">{m}</span>
                </label>
              ))}
            </div>
          )}
