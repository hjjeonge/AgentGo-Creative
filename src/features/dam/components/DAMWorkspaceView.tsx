...
export const DAMWorkspaceView: React.FC<Props> = ({
  tasks,
  onTaskClick,
  onTaskStatusChange,
}: Props) => {
  ...
  const getStatusBadge = (status: DAMTask['status']) => {
    ...
    return (
      <span
        className={`px-[8px] py-[2px] rounded-full text-[11px] font-bold uppercase ${styles[status]}`}
      >
        {status}
      </span>
    );
  ...
  const getPriorityBadge = (priority: DAMTask['priority']) => {
    ...
    return (
      <span
        className={`px-[6px] py-[2px] rounded text-[10px] font-bold uppercase ${styles[priority]}`}
      >
        {priority}
      </span>
    );
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#F8FAFC] border-b border-[#E2E8F0]">
                  ...
                  <th className="px-[12px] py-[12px] text-[12px] font-bold text-[#475569] uppercase tracking-wider">
                    Title
                  </th>
                  <th className="px-[12px] py-[12px] text-[12px] font-bold text-[#475569] uppercase tracking-wider">
                    Status
              ...
              <tbody>
                {filteredTasks.length > 0 ? (
                    ...
                    >
                      ...
                      <td className="px-[12px] py-[14px]">
                        <div className="flex flex-col">
                          <span className="text-[13px] font-bold text-[#0F172B] group-hover:text-[#155DFC]">
                            {task.title}
                          </span>
                          <span className="text-[12px] text-[#94A3B8] line-clamp-1">
                            {task.description}
                          </span>
                        </div>
                      </td>
                  ...
                  <tr>
                    <td
                      colSpan={6}
                      className="px-[12px] py-[60px] text-center text-[14px] text-[#94A3B8]"
                    >
                      할당된 작업이 없습니다.
                    </td>
                  </tr>
        ...
        <aside className="w-[320px] border-l border-[#E2E8F0] bg-[#F8FAFC] p-[20px] flex flex-col gap-[14px]">
          ...
          <div className="p-[10px] border border-[#E2E8F0] rounded-[8px] bg-white">
            ...
            <div className="flex flex-wrap gap-[6px]">
              {(
                ['todo', 'in-progress', 'review', 'done'] as DAMTask['status'][]
              ).map((status) => (
                <button
                  key={status}
