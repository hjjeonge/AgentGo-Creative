...
export const FileDetailModal: React.FC<Props> = ({
  ...
  const [smartCropRatio, setSmartCropRatio] = useState<
      ...
      return file.url ? (
        <div className="relative w-full">
          <video
            ...
            src={file.url}
            controls
            className="max-w-full max-h-[400px] rounded-[8px] w-full"
          />
    ...
    if (file.type === 'pdf') {
      return file.url ? (
        <iframe
          src={file.url}
          title={file.name}
          className="w-full h-[400px] rounded-[8px] border border-[#E2E8F0]"
        />
      ) : (
        <div className="w-full h-[300px] bg-[#F1F5F9] rounded-[8px] flex flex-col items-center justify-center gap-[12px]">
  ...
  const renderBasicTab = () => (
    <div className="flex flex-col gap-[4px]">
      {[
        ...
        <div
          key={item.label}
          className="flex border-b border-[#F1F5F9] py-[10px]"
        >
          <div className="w-[120px] shrink-0 text-[13px] text-[#64748B]">
            {item.label}
          </div>
          <div className="flex-1">
            {editMode && !item.readonly ? (
              ...
              <span
                className={`text-[13px] ${item.label === 'Status' ? getStatusColor(item.value) : 'text-[#0F172B]'}`}
              >
                {item.value || '-'}
              </span>
            )}
    ...
    <div className="flex flex-col gap-[4px]">
      {[
        ...
        >
          ...
          <div className="flex-1 text-[13px] text-[#0F172B]">
            {item.value || '-'}
          </div>
        </div>
      ))}
    ...
    <div className="flex flex-col gap-[20px]">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-[#F8FAFC] border-b border-[#E2E8F0]">
              <th className="px-[12px] py-[10px] text-[12px] font-semibold text-[#475569] w-[100px]">
                Category
              </th>
              <th className="px-[12px] py-[10px] text-[12px] font-semibold text-[#475569] w-[120px]">
                Value
              </th>
              <th className="px-[12px] py-[10px] text-[12px] font-semibold text-[#475569]">
                Reason
              </th>
            </tr>
          </thead>
            ...
            {(file.aiTags || []).length > 0 ? (
              file.aiTags!.map((tag, i) => (
                <tr key={i} className="border-b border-[#F1F5F9] last:border-0">
                  <td className="px-[12px] py-[10px] text-[13px] text-[#64748B]">
                    {tag.category}
                  </td>
                  <td className="px-[12px] py-[10px] text-[13px] text-[#0F172B] font-medium">
                    {tag.value}
                  </td>
                  <td className="px-[12px] py-[10px] text-[12px] text-[#94A3B8] italic">
                    <div className="inline-flex items-center gap-[6px] group/reason relative">
                      ...
                      <span className="w-[16px] h-[16px] rounded-full border border-[#CBD5E1] text-[#64748B] text-[10px] font-bold inline-flex items-center justify-center cursor-help">
                        i
                      </span>
              ...
              <tr>
                <td
                  colSpan={3}
                  className="px-[12px] py-[20px] text-center text-[13px] text-[#94A3B8]"
                >
                  AI 분석 데이터가 없습니다.
                </td>
              </tr>
      ...
      {file.aiSummary && (
        <div className="bg-[#F1F5F9] p-[16px] rounded-[8px]">
          <p className="text-[12px] font-semibold text-[#475569] mb-[8px]">
            AI Summary
          </p>
    ...
    return (
      <div className="flex flex-col gap-[4px]">
        {[
          ...
          <div
            key={item.label}
            className={`flex border-b border-[#F1F5F9] py-[10px] ${item.full ? 'flex-col gap-[4px]' : ''}`}
          >
            <div
              className={`${item.full ? 'w-full' : 'w-[120px]'} shrink-0 text-[13px] text-[#64748B]`}
            >
              {item.label}
            </div>
            <div className="flex-1 text-[13px] text-[#0F172B]">
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-[16px] shadow-2xl w-[960px] max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between p-[16px_24px] border-b border-[#E2E8F0]">
          <h2 className="text-[16px] font-bold text-[#0F172B] truncate max-w-[60%]">
            {file.name}
          </h2>
          <div className="flex items-center gap-[10px]">
            ...
            <button
              onClick={onClose}
              className="p-[4px] hover:bg-[#F1F5F9] rounded-full transition-colors"
            >
            ...
            {file.referenceImages && file.referenceImages.length > 0 && (
              <div>
                <p className="text-[12px] text-[#475569] font-bold mb-[10px] flex items-center gap-[6px]">
                  레퍼런스 이미지
                  <span className="text-[10px] px-[6px] py-[2px] bg-[#EFF6FF] text-[#155DFC] rounded-full">
                ...
                <div className="flex flex-wrap gap-[10px]">
                  {file.referenceImages.map((src, i) => (
                    <div
                      key={i}
                      className="group relative w-[90px] h-[90px] rounded-[8px] overflow-hidden border border-[#E2E8F0] hover:border-[#155DFC] transition-colors cursor-pointer"
                    >
                      <img
                        src={src}
                        alt={`ref-${i}`}
                ...
                editMode ? (
                  <>
                    <button
                      ...
                      className="px-[24px] py-[10px] border border-[#CBD5E1] text-[#475569] rounded-[8px] text-[14px] font-medium hover:bg-[#F1F5F9]"
                    >
                      취소
                    </button>
                    <button
                      onClick={handleSave}
                      className="px-[24px] py-[10px] bg-[#155DFC] text-white rounded-[8px] text-[14px] font-bold shadow-md hover:bg-[#155DFC]/90 active:scale-[0.98]"
                    >
                      저장
                    </button>
                  </>
                  ...
                  <>
                    <button
                      ...
                      className="px-[24px] py-[10px] border border-[#155DFC] text-[#155DFC] rounded-[8px] text-[14px] font-medium hover:bg-[#EFF6FF]"
                    >
                      편집
                    </button>
                    <button
                      onClick={onClose}
                      className="px-[24px] py-[10px] bg-[#155DFC] text-white rounded-[8px] text-[14px] font-bold hover:bg-[#155DFC]/90 active:scale-[0.98]"
                    >
                      확인
                    </button>
                  </>
